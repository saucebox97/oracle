...
-- 3번째에 6칸넣어야돼는데 4칸넣어도 2칸이 공백처리됐다.
...
-- DELETE FROM char_compare WHERE sn = '103';
...
SELECT 
    REPLACE(char_4, ' ', '+') AS char_4, -- replace는 내부데이터안바꾸고 보여줄때만 바뀜
    REPLACE(char_6, ' ', '+') AS char_6 -- 그래서 비교할때 내부데이터안바껴서상관없음
FROM char_compare
WHERE sn = '101' 
    AND char_4 = char_6 -- char은 공백무시하고 비교 즉 같음 /단 앞,가운데공백은처리됌
;
...
FROM char_compare
WHERE sn = '102'
    AND char_4 > char_6 -- 4번째 글자인 D가 A보다 커서 참이된다
;
...
SELECT 
    REPLACE(char_4, ' ', '+') AS char_4, -- 보여줄때만 값보여서보여줌 실제 데이터를안바뀜
    REPLACE(varchar_6, ' ', '+') AS varchar_6
FROM varchar_compare
WHERE sn = '101' -- artchar2 은 공백도 포함해서 비교한다
    AND char_4 = TRIM(varchar_6) -- 비교할때만 공백을제거하고 비교 값은안변함
;
...
FROM varchar_compare
WHERE sn = '101'
    AND char_4 = 'SQLD         ' -- 둘이 같다 상수는 앞의 타입하고같은 타입으로비교
;
...
FROM varchar_compare
WHERE sn = '101'
    AND varchar_6 = 'SQLD' -- 둘이 다르다 상수는 앞의 타입하고같은 타입으로비교 
;
...
SELECT
    ...
    TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일"') AS "날짜 - 한글포함", -- ""빼면 에러남 한글쓸거면 닫아야됌
