...
-- HAVING : 그룹화된 결과에서 조건을 걸어 행 수를 제한 / 그룹화 후에 하는 거여서 집계합수됌
-- 일반적으로 GROUP BY 후에 씀
-- 부서별로 가장 어린사람의 생년월일, 연장자의 생년월일, 부서별 총 사원 수를 조회
-- 그런데 부서별 사원이 1명만 부서의 정보는 조회하고 싶지 않음.
SELECT 
    dept_cd
    , MAX(birth_de) 최연소자
    , MIN(birth_de) 최연장자
...
SELECT
    ...
    , dept_cd
FROM tb_emp
ORDER BY dept_cd, sex_cd -- dept_cd와 sex_cd 같은것들끼리 통계를 냄
;
...
SELECT
    ...
    , addr
FROM tb_emp
ORDER BY emp_no DESC -- 오름차는 기본이기떄문에 표기를안해도 내림차는 표기해야됀다
;
...
SELECT
    ...
    , addr
FROM tb_emp
ORDER BY emp_nm DESC -- 이름으로정렬은 유니코드로 정렬 a-z 가-히
;
...
SELECT
    ...
    , dept_cd
FROM tb_emp
ORDER BY 3 ASC, 1 DESC -- 컬럼으로도 할 수있다 dept_cd - 3 emp_nm - 2
;
...
SELECT
    ...
    , dept_cd
FROM tb_emp
ORDER BY 3 ASC, emp_no DESC -- 컬럼하고 숫자 별칭 섞어서써도됀다
;
...
SELECT 
    emp_no "사번" -- 그룹바이에서 지정한것만쓸 수 있다
    , SUM(pay_amt) 연봉
FROM tb_sal_his
WHERE pay_de BETWEEN '20190101' AND '20191231'
GROUP BY emp_no
HAVING AVG(pay_amt) >= 4500000 
ORDER BY emp_no -- 그룹바이에서 지정한것만쓸 수 있다
;
