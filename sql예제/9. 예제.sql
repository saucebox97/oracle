...
-- 3. 첫번째 쿼리의 열(컬럼)의 개수와 타입이 두번째 쿼리의 열(컬럼)수와 타입과 동일해야 함.
-- 4. 자동으로 정렬이 일어남 (첫번째 컬럼 오름차가 기본값)

-- brith_de은 출력하고 오른쪽에있다
SELECT
...
FROM tb_emp
WHERE birth_de BETWEEN '19700101' AND '19791231'
ORDER BY BD DESC -- 다 끝나고 우선순위해야됌
;
...
START WITH A.direct_manager_emp_no IS NULL -- 김회장이 NULL
CONNECT BY PRIOR A.emp_no = A.direct_manager_emp_no
ORDER SIBLINGS BY A.emp_no DESC -- 형제 정렬
;
